  ...
  <div>
    <div v-for="(item, index) of diaryValue" :key="index" class="typeDiary-kuang">
      <div>
        ...
        <div class="cust-info">
          <img src :style="{ backgroundImage: `url(${item.authorIcon})`}">
          <span>{{item.authorName}}</span>
        </div>
        <div class="diary-cont">
          <span class="dirays" v-if="!diaryInfoShow" v-html="diaryValue.introducePart"></span>
          <span class="dirays" v-else v-html="diaryValue.introduce"></span>
          <span
            class="more"
            @click="diaryShow"
            v-if="diaryValue.introducePart"
          >{{ !diaryInfoShow ? '更多' : '收起' }}></span>
        </div>
        ...
        <div class="typeDiary-bottom flex-r img-show">
          ...
          <div class="flex-r show-num zan-add">
            ...
            <div span="4" @click="giveGoods(item)">
              <span
    ...
    <div v-if="diaryValue.length == 0" class="nodata">
...
export default {
  ...
  methods: {
    // 日记显示收起
    diaryShow() {
      this.diaryInfoShow = !this.diaryInfoShow;
    },
    jump(item) {
...
.typeDiary-kuang {
  background: #fff;
  margin-bottom: 10px;
  padding: 25px 0.24px * 100 0 0.24px * 100;
  width: 100%;
