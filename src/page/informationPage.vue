<template>
  <div class="information" @click="betterScrollClick">
    <div class="information-page-content-box" ref="boxContent" @touchstart="boxTouchStart" @touchmove="boxTouchMove" @touchend="boxTouchEnd">
    <!-- <vue-better-scroll
      class="wrapper"
      ref="scroll"
      :click="true"
    ...
    > -->
      <div v-if="topLoading" class="loading-text">加载中...</div>
      ...
      <div v-if="bottomLoading" class="loading-text">加载中...</div>
    <!-- </vue-better-scroll> -->
    </div>
    <writeComment
      @send="getList(id)"
...
export default {
  ...
  // computed: {
  //   isRefush () {
  //     if (this.scrollTopStart == 0 && this.scrollTopEnd == 0) {
  //       return true
  //     }
  //     return false
  //   }
...
// }
  ...
  computed: {
    ...
    checkSource () {
      ...
      if (!this.shared) {
        ...
        if (!a) { // 原生浏览器内处理逻辑
          ...
          window.location.href = `http://img.meiduduo.com/h5/download.html?path=newsdetail&id=${self.$route.query.id}&type=${self.$route.query.type}&share=0&inviter=${this.custId}&customerPacketId=${self.$route.query.customerPacketId}&imgUrl=${self.img[0]}&titleName=${self.information.title}&token=${self.$route.query.token}`
    ...
    scrollPosition (type) {
      let a = this.$refs.boxContent
      console.log('scrollTop', a.scrollTop)
      if (type == 'start') {
        this.scrollTopStart = a.scrollTop
      } else if (type == 'end') {
        this.scrollTopEnd = a.scrollTop
      }
      // console.log(x, y)
    },
    boxContentHeight () {
      let a = this.$refs.listContent
      console.log('listContent', a.clientHeight)
      this.boxHeight = a.clientHeight
    },
    contentHeight () {
      let a = this.$refs.boxContent
      console.log('boxContent', a.clientHeight)
      this.contentHeights = a.clientHeight
    },
    boxTouchStart (event) {
      console.log('boxTouchStart')
      console.log(event)
      console.log(event.changedTouches[0].pageY)
      this.pageYStart = event.changedTouches[0].pageY
    ...
    boxTouchEnd (event) {
      console.log('boxTouchend')
      console.log(event)
      console.log(event.changedTouches[0].pageY)
      this.pageYEnd = event.changedTouches[0].pageY
      let a = this.pageYEnd - this.pageYStart
      this.scrollPosition('end')
      if (this.scrollTopStart == 0 && this.scrollTopEnd == 0 && a > 50) {
        console.log('下拉刷新')
        this.topLoading = true
        this.onPullingDown()
      }
      if ((this.scrollTopEnd + this.contentHeights) > this.boxHeight) {
        console.log('上拉加载')
