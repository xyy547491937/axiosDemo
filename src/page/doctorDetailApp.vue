<template>
  <div class="doctor-detail">
    ...
    <div v-if="servicePersonalData.organType == 1" @click="toDoctorSaid" class="doctor-messages box-margin-bottom">
      <span>医生说说</span>
    ...
    <div v-if="comments.length > 0 && servicePersonalData.staffId" class="doctor-commtents box-margin-bottom">
      <div class="commtents-header">
        ...
        <span @click.stop="allComments">{{ commentNumber }}条评价></span>
      </div>
    ...
    <div class="product-case box-margin-bottom">
      ...
      <p @click="toAllExample" class="watch-all box-border-top">查看全部 {{ exampleTotal }}个案例 ></p>
    </div>
    <div class="product-selling box-margin-bottom">
      ...
      <p @click="toAllHotProject" class="watch-all box-border-top">查看全部 {{ hotTotal }}个特惠 ></p>
    </div>
...
export default {
  ...
  methods: {
    ...mapMutations(['updateDoctorDetail', 'updateDoctorFoucs', 'saveUserId', 'saveToken']),
    toDoctorSaid () { // 跳转到医生说说页
      const navigatorInfo = navigator.userAgent;
      const isAndroid =
        navigatorInfo.indexOf("Android") > -1 ||
        navigatorInfo.indexOf("Adr") > -1; //android终端
      ...
      this.updateDoctorDetail({
        ...
        icon: this.servicePersonalData.icon,
        id: this.servicePersonalData.id
      })
      this.$router.push({
        path: '/doctorSaid',
        query: {
    ...
    query() {
      ...
      if ((this.$route.query.userId && this.$route.query.userId != '-1') || this.userId) {
        params.custId = this.userId || this.$route.query.userId
      }
      queryDoctorDetail(params).then(res => {
        if (res.code === 1) {
          ...
          if (res.data.isFollow) {
            this.updateDoctorFoucs(true)
          } else {
            this.updateDoctorFoucs(false)
          }
    ...
    focus(val) {
      ...
      toAttention({
        ...
        custId: this.userId || this.$route.query.userId,
        recordType: 1,
        state: val ? -1 : 1,
        token: this.$route.query.token == '-1' ? null : this.$route.query.token
      }).then(res => {
    ...
    toOrganPage() {
      ...
      // let params = {
      //   lat: this.servicePersonalData.lat,
      //   lng: this.servicePersonalData.lng
      // };
      let params = {
    ...
    toAllExample () {
      ...
      let params = {
        ...
        peopleId: this.servicePersonalData.id
      }
      const navigatorInfo = navigator.userAgent;
      const isAndroid =
        navigatorInfo.indexOf("Android") > -1 ||
      ...
      const isiOS = !!navigatorInfo.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/); //ios终端
      if (isAndroid) {
        window.android.lookAllCase(JSON.stringify(params));
      }
      if (isiOS) {
        window.webkit.messageHandlers.lookAllCase.postMessage(params);
      }
    },
    toHotProject (item) {
      if (!this.checkSource()) {
    ...
    toAllHotProject () {
      ...
      const isiOS = !!navigatorInfo.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/); //ios终端
      if (isAndroid) {
        window.android.lookAllDiscountProject(JSON.stringify(params));
      }
      if (isiOS) {
        window.webkit.messageHandlers.lookAllDiscountProject.postMessage(params);
      }
    },
    allComments () {
      if (!this.checkSource()) {
      ...
      const isiOS = !!navigatorInfo.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/); //ios终端
      if (isAndroid) {
        window.android.moreComment(JSON.stringify(params));
      }
      if (isiOS) {
        window.webkit.messageHandlers.moreComment.postMessage(params);
      }
    },
    checkSource() {
      const self = this;
