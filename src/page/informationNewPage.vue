<template>
  <div class="information">
    <div v-if="showOpen" class="top-recomment" @click="betterScrollClick">
      <div class="logo-box">
        <div class="logo"></div>
        <div class="slogo">女人都爱逛胚豆</div>
      </div>
      <div class="openbtn" @click.stop="checkSource">打开</div>
    </div>
    <vue-better-scroll
      class="wrapper"
      ref="scroll"
    ...
    >
      <div
        ...
        id="main"
        ref="listContent"
        @click="betterScrollClick"
        v-longTouch="handleLongTouch"
      >
        <div class="swiper-img-box">
          <span class="swiper-img-mask"></span>
          <swiper
            v-if="coverImgList.length > 1"
            :list="coverImgList"
            :show-dots="false"
            :loop="true"
          ...
          <div
            v-if="coverImgList.length == 1"
            class="single-swiper-img"
            :style="{backgroundImage:`url('${coverImgList[0].img}')`}"></div>
          <span v-if="coverImgList.length > 1" class="swiper-img-index">{{ nowCurrentImgIndex }} / {{ coverImgList.length }}</span>
        </div>
        <!-- <div class="title">{{information.title}}</div> -->
        <div class="article-user-infos">
          <span
            class="article-user-icon"
            v-if="information.title"
            @click.stop="type != 4 ? goUserpage(information.author,information) : ''"
            :style="{backgroundImage:`url('${information.authorIcon ? information.authorIcon : 'http://img.meiduduo.com/images/h5/default_icon.png'}')`}"></span>
          <span class="article-user-name">{{information.authorName}}</span>
          <span class="article-user-time">{{information.releaseTime ? information.releaseTime: ''}}</span>
        </div>
        <div class="title">{{information.title}}</div>
        <!-- <div class="infor_top_detail">
          ...
          <span
            ...
            @click.stop="focusRecordSave(information)"
          >{{information.isFollow==1 ? '已关注' : '+关注'}}</span>
        </div> -->
        <div class="content" @click.stop="openImg">
          <!--  视屏区域 -->
        ...
        <div class="article-source">文章来源：{{ information.source }}</div>

        <!-- // ! 收藏点赞模块 -->
        <!-- <div v-if="information.title" class="operation">
          <span
            class="fabulous"
            @click.stop="publicMth('isThumbsUp',customerRecordSave,information)"
          >
            <i :class="'iconfont '+ (information.isThumbsUp?'icon-zan icon-zanok':'icon-zan')"></i>
          ...
          <span
            class="collection"
            v-if="type!=8&&type!=10"
            @click.stop="publicMth('isFavourite',storeRecordSave,information)"
          >
        ...
        <!-- 推荐阅读 -->
        <!-- TODO: 推荐阅读隐藏 -->
        <!-- <div class="propose-read" v-if="false"> -->
        <div class="propose-read" v-if="type!=8 && recommendInfoDTOList.length>0 ">
          <div class="read-box">
            <div
              ...
              :key="i"
              @click.stop="proposeArticle(v)"
            >
              <p class="propose-title">{{v.title}}</p>
              <div class="propose-img" :style="{backgroundImage:`url('${v.cover[0]}')`}"></div>
            </div>
          </div>
        </div>
        ...
        <!-- 回复 -->
        <div
          v-if="comments.length > 0 && information.title"
          class="information_review"
        ...
        >
          <div class="review_main">
            <div class="review_head">
              ...
              <figure class="head" @click.stop="goUserpage(item.custId,item)">
            ...
            <div class="review_content">
              <div class="review_title">
                ...
                <span class="review_num" ref="zan" @click.stop="commentsZan(item)">
              ...
              <p class="review_other">
                ...
                <span class="detailC" @click.stop="writeShow(item)">回复</span>
              </p>
            </div>
          </div>
        </div>
    ...
    ></writeComment>-->
    <writeComment
      @send="pageNum=1;getList(id)"
      @sendStatusM="sendStatusM"
      @sendStatusM1="sendStatusM1"
      @showInput="showInput"
...
import { Scroller, Confirm, TransferDom, Loading, Previewer, Swiper } from "vux";
...
export default {
  data() {
    return {
      ...
      coverImgList: [
        {
          img: 'http://fs.meiduduo.com/sys//sys/article/singleCover//1548908324641.jpg'
        },
        {
          img: 'http://fs.meiduduo.com/sys//sys/article/multiCover//1551336371471.jpg'
        },
        {
          img: 'http://fs.meiduduo.com/sys//sys/article/multiCover//1551336389487.jpg'
        },
        {
          img: 'http://fs.meiduduo.com/sys//sys/article/multiCover//1551336403378.jpg'
        },
      ], // 封面图列表
      nowCurrentImgIndex: 1,
    };
  },
  components: {
    writeComment,
  ...
  created() {
    ...
    console.log("进入的链接", location.href);
    ...
    if (
      ...
      this.$route.query.customerPacketId != -1
    ) {
      // if (isiOS) {
  ...
  methods: {
    ...
    querySignatureData() {
      querySignature({
        ...
        if (res.code == 1) {
          ...
          this.signatureData.title = this.information.title;
          this.signatureData.imgUrl = this.img[0];
    ...
    iconerror(event) {
      event.target.style.backgroundImage =
        "url(http://img.meiduduo.com/images/h5/default_icon.png)";
      event.target.src = "";
    },
    beforeScrollStart(e) {
    ...
    queryOneByPkAndUserId(id) {
      ...
      http("article/queryOneByPkAndUserId", option).then(res => {
        ...
        if (res.code == 1) {
          ...
          let imgarr = res.data.contentImgs
            ? res.data.contentImgs.split(",")
            : [];
          ...
          this.img = this.information.cover
            ? this.information.cover.split(",")
            : [];
    ...
    publicMth(data, mth, item) {
      ...
      console.log(item["isThumbsUp"]);
      if (data == "isThumbsUp") {
        this.$set(item, "isThumbsUp", item["isThumbsUp"] == 1 ? 0 : 1);
        this.$set(
          item,
          "fabulousNum",
          item["isThumbsUp"] > 0 ? item.fabulousNum + 1 : item.fabulousNum - 1
        );
        mth(item, item["isThumbsUp"]);
      }
      if (data == "isFavourite") {
        this.$set(item, "isFavourite", !item["isFavourite"]);
        this.$set(
          item,
          "favouriteNum",
          item["isFavourite"] ? item.favouriteNum + 1 : item.favouriteNum - 1
        );
        mth(item, item["isFavourite"]);
      }
    },
    onConfirm() {
      console.log("点击确认");
    ...
    proposeArticle(item) {
      ...
      // console.log(params);
      if (isAndroid) {
        window.android.articleOrNewsDetail(JSON.stringify(params));
      }
      if (isiOS) {
        window.webkit.messageHandlers.articleOrNewsDetail.postMessage(params);
      }
    },
    commentsZan(item) {
      if (!this.checkSource()) {
    ...
    writeShow(item) {
      console.log(item);
      this.$set(this.writeItem, "commentText", "reply");
    ...
    sendStatusM1(data) {
      console.log(data, 3);
      this.$set(this.writeItem, "focus", data);
      this.$set(this.writeItem, "commentText", "add");
    },
    blurChange() {
      this.$set(this.writeItem, "focus", false);
      this.$set(this.writeItem, "place", "亲，说两句...");
    ...
    checkSource() {
      ...
      if (!this.shared) {
        ...
        if (!a) {
          ...
          // 微信浏览器内处理逻辑
          window.location.href = `http://img.meiduduo.com/h5/download.html?path=newsdetail&id=${
            self.$route.query.id
          }&type=${self.$route.query.type}&share=0&inviter=${
            this.custId
          }&customerPacketId=${self.$route.query.customerPacketId}&imgUrl=${
            self.img[0]
          }&titleName=${self.information.title}&token=${
    ...
    contentClick(event) {
      ...
      // return
      // TODO: 隐藏超链接跳转
      let type = event.target.type || event.target.parentElement.type;
      ...
      if (type == 4) {
        ...
        if (isAndroid) {
          window.android.activityDetail(JSON.stringify(params));
        }
        if (isiOS) {
          window.webkit.messageHandlers.activityDetail.postMessage(params);
        }
      } else if (type == 3) {
        // 商品
        // let comm = {}
        // http('commodity/getCommodityInfo', {
        //   id: id,
        //   userId: this.$route.query.userId != -1 ? this.$route.query.userId : '',
        //   token: this.$route.query.token != -1 ? this.$route.query.token : ''
        // }).then(res => {
        //   if (res.code == 1) {
        //     comm = res.data.comm
        //     comm.commImg = comm.commImg ? comm.commImg.split(',') : []
        //   }
        // })
        let params = {
          commId: id
          // commName: comm.commName ? comm.commName : '',
          // remark: comm.remark ? comm.remark : '',
          // commImg: comm.commImg[0]
        };
        if (isAndroid) {
          window.android.projectDetail(JSON.stringify(params));
        }
        if (isiOS) {
          window.webkit.messageHandlers.projectDetail.postMessage(params);
        }
      } else if (type == 2) {
        // 服务人员
        let params = {
    ...
    swiperIndexChange(currentIndex) {
      ...
      this.nowCurrentImgIndex = currentIndex + 1
    }
  }
};
</script>
...
.swiper-img-mask {
  ...
  top: 0;
  left: 0;
  background-image: url('http://img.meiduduo.com/images/h5/swiper_mask.png');
  background-repeat: no-repeat;
